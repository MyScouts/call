...
class _TeamDetailScreenState extends State<TeamDetailScreen> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: BlocConsumer<TeamDetailBloc, TeamDetailState>(
        ...
        buildWhen: (prev, current) => current is FetchTeamDetailSuccess || current is FetchTeamsMemberSuccess || current is LoadingTeamDetail,
        builder: (context, state) {
          ...
          final canUpdateMembers = myId == widget.bossGroupId || myId == team?.boss?.id;

          final isBossGroupButNotBossTeam = myId == widget.bossGroupId && myId != team?.boss?.id;
          final isBossGroupAndBossTeam = myId == widget.bossGroupId && myId == team?.boss?.id;
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          if (!canUpdateMembers && isMember) _askToLeaveBtn(teamId: '${team?.id}'),
                          const SizedBox(height: 20),
                          _membersWidget(members: members, canUpdateMembers: canUpdateMembers),
                        ],
                      ),
                    ),
                  ),
  ...
  Widget _teamHeaderWidget(Team? team) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: Colors.black.withOpacity(.07), blurRadius: 16, offset: const Offset(0, 4))],
      ),
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
            ...
            children: [
                        ...
                        child: SizedBox(
                          ...
                          child: Text(
                            ...
                            style: Theme
                                ...
                                .textTheme
                                .bodyLarge
                                ?.copyWith(fontSize: 14, fontWeight: FontWeight.w400, height: 1.5, color: const Color(0xFF353DFF)),
                          ),
                        ),
                      ),
                    ],
              ...
              Text(
                ...
                style: Theme
                    ...
                    .textTheme
                    .bodyLarge
                    ?.copyWith(fontSize: 14, fontWeight: FontWeight.w500, height: 1.5, color: const Color(0xFFACACAC)),
              ),
            ],
          )
        ],
                      ...
                      Padding(
                        ...
                        child: Text(
                          ...
                          style: Theme
                              ...
                              .textTheme
                              .bodyLarge
                              ?.copyWith(color: const Color(0xFF4B84F7), fontWeight: FontWeight.w500, fontSize: 14),
                        ),
                      )
                    ],
                  ),
...
Widget _askToLeaveBtn({required String teamId}) {
  return GestureDetector(
    ...
    child: Container(
      ...
      height: 48,
      decoration: BoxDecoration(color: const Color(0xFFFFEEEC), borderRadius: BorderRadius.circular(10)),
      child: RichText(
        text: TextSpan(
          text: '',
          children: <InlineSpan>[
...
Widget _membersWidget({required List<User> members, required bool canUpdateMembers}) {
  return Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      Text(
      ...
      ...members.map(
            ...
            TeamMemberWidget(
              ...
              trailing: canUpdateMembers && member.id != myId
                ...
                icon: const Icon(Icons.more_horiz, color: Color(0xFF212121)),
...
void _onTeamDetailBlocListen(BuildContext context, TeamDetailState state) {
  if (state is GetLeaveTeamStatusLoading) {
    ...
    context.hideLoading();
    if (state.requests.requests != null && state.requests.requests!.isNotEmpty) {
      final dayLeft = state.requests.requests!.first.createdAt!.add(const Duration(days: CommunityConstant.dayForLeaveTeamRequest + 1)).dayLeft();
