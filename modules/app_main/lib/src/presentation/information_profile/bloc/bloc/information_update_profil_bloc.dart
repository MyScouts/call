...
@injectable
class InformationUpdateProfilBloc
    extends Bloc<InformationEvent, InformationUpdateProfilState> {
  final UserUsecase _userUsecase;
  final InformationPDoneSharePreferencesUsecase
      _informationPDoneSharePreferencesUsecase;

  InformationUpdateProfilBloc(
      this._userUsecase, this._informationPDoneSharePreferencesUsecase)
  ...
  Future<void> updatePDoneProfile(InformationUpdateProfilEvent event,
      Emitter<InformationUpdateProfilState> emit) async {
  ...
  Future<void> updateNonePDoneProfile(InformationNoneUpdateProfilEvent event,
      Emitter<InformationUpdateProfilState> emit) async {
    if (state is InformationUpdateProfilLoading) return;
    try {
      emit(InformationUpdateProfilLoading());
      // final response = await _userUsecase.updateNonePNoneDoneProfile(event.updateNonePDoneProfilePayload!);
      // await _informationPDoneSharePreferencesUsecase
      //     .saveInfoNonePDoneProfile(response.profile);
      final values =
          _informationPDoneSharePreferencesUsecase.getInfoNonePDoneProfile();
  ...
  Future<void> getPDoneProfile(GetInformationPDoneProfileEvent event,
      Emitter<InformationUpdateProfilState> emit) async {
    try {
      emit(GetInformationPDoneProfileLoading());
      await _userUsecase.getPDoneProfile().then(
          (value) => emit(GetInformationPDoneProfileSuccess(user: value)));
    } catch (error) {
