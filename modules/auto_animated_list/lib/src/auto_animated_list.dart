...
class _AutoAnimatedListState<T> extends State<AutoAnimatedList<T>> {
  ...
  void _handleListUpdated(List<T> oldList) {
    ...
    for (final update in updates) {
      update.when(
        ...
        remove: (from, _) => _handleRemoveItem(from),
        change: (_, __) {},
      );
    }
  }

  void _handleInsertItem(int to) {
    return _listKey.currentState?.insertItem(
      to,
  ...
  void _handleRemoveItem(
    ...
    return _listKey.currentState?.removeItem(
      ...
      duration: useAnimation ? widget.duration : Duration.zero,
    );
  }
}
